...
const Cart = () => {
	...
	const handlers = {
		...
		purchase: (e) => {
			...
			UserService.makePurchase(purchaseDetails, bonusPoints)
				.then((res) => {
			...
			<div className={styles.container}>
				<div className={styles.contain}>
					<div className={styles.topBar}>
						...
						<div className={styles.info} onClick={() => showInfoBox(styles.open)} />
						{cart.length !== 0 && (
							<button form='CARTFORM' onClick={(e) => handlers.displayPurchaseModel(e)}>
								Rent
							</button>
						)}
					</div>
					{isDataLoaded ? (
						cart.length !== 0 ? (
							...
							<div style={{ textAlign: "center" }}>Looks like your cart is empty..</div>
						)
					) : (
...
export const CartItem = ({ movieDetails, updateAmount }) => {
	...
	const handler = {
		calAmount: (value) => {
			let amt = (limit === 0 ? fee * value : fee) + (value - limit > 0 && limit !== 0 ? value - limit : 0) * fee;
	...
	<div className={styles.infoContainer + " " + info}>
		<div className={styles.model}>
			<div className={styles.content}>
				...
				<ul>
					<li>
						<img src={PremiumIcon} alt='New Movie' />
						<span>Premium fee times number of days rented.</span>
					</li>
					<li>
						<img src={RegularIcon} alt='Regular Movie' />
						<span>Regular fee once for the first 3 days plus Regular fee times the number of days over 3.</span>
					</li>
					<li>
						<img src={OldIcon} alt='Old Movie' />
						<span>Regular fee once for the first 5 days plus Regular fee times the number of days over5.</span>
					</li>
				</ul>
			</div>
			<div className={styles.content}>
